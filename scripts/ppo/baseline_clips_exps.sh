#!/bin/bash
#SBATCH --job-name=mlp_walker_stand    # Base job name
#SBATCH --output=logs/%x/%j.out        # Log file (%x=job-name, %j=job-ID)
#SBATCH --error=logs/%x/%j.err         # Error log                    
#SBATCH --gres=gpu:1                # Requesting 4 GPUs for parallel jobs
...
pwd
echo "Beginning mlp clip_coef sweep across 2 GPUs"
...
for clip_coef in "${CLIP_COEFS[@]}"; do
    ...
    RUN_NAME="mlp_clip_coef_${clip_coef}"
    ...
    CUDA_VISIBLE_DEVICES=${GPU_ID} python online_training.py \
        ++env_cls_route.truncation_limit=1000 \
        ++trainer.cfg.num_epochs=4 \
        ++agent.transformer_cfg.clip_coef=${clip_coef} \
        ++agent.name="${RUN_NAME}" \
        exp_name="${RUN_NAME}" \
        project=mlp_ppo_walker \
        notes="walker_stand_${clip_coef}" &
