...
from agents.maskdp.rewards_joint_PE import MaskDPJointPE

class AgentWrapper:
    mdp_model   : MaskDPJointPE
    def __init__(self, agent: MaskDPJointPE):
        self.agent = agent
    
    def __call__(self, state, std):
        pass
...
class MaskDPActorPPOPolicy(ActorCriticPolicy):
    ...
    def __init__(self, *args, maskdp_agent: MaskDPJointPE, fixed_std: float=0.1, **kwargs):
        super().__init__(*args, **kwargs)
        self.maskdp_actor = AgentWrapper(maskdp_agent)
    ...
    def _get_action_dist_from_latent(self, latent_pi, latent_sde=None):
        ...
        # (batch, state_dim) 
        # barch is always 1 here though, or will it?
