...
def get_dir(cfg):
    ...
    snapshot = resume_dir / str(cfg.seed) / cfg.algorithm / \
               f"snapshot_{cfg.resume_step}.pt"
...
@hydra.main(config_path=".", config_name="finetune_exorl")
def main(cfg):
    work_dir = Path.cwd()
    ...
    exp_name = "_".join([
        cfg.agent.name, domain, str(cfg.seed), str(cfg.algorithm)])
    ...
    replay_train_dir = \
        Path(cfg.replay_buffer_dir) / cfg.task / "val"
